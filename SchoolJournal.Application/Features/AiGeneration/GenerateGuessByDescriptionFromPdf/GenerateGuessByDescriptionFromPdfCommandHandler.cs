...
public sealed class GenerateGuessByDescriptionFromPdfCommandHandler(
    IPdfTextExtractionService pdfExtractor,
    IAiQuizGenerator aiQuizGenerator)
    : IRequestHandler<GenerateGuessByDescriptionFromPdfCommand, ErrorOr<GeneratedQuizResponse>>
{
    private const int MaxTextLengthForAi = 300_000;

    public async Task<ErrorOr<GeneratedQuizResponse>> Handle(GenerateGuessByDescriptionFromPdfCommand request, CancellationToken cancellationToken)
    {
        ...
        return await aiQuizGenerator.GenerateGuessByDescriptionAsync(
            extractedText,
