...
public sealed class GenerateAssociativeBushFromPdfCommandHandler(
    IPdfTextExtractionService pdfExtractor,
    IAiQuizGenerator aiQuizGenerator)
    : IRequestHandler<GenerateAssociativeBushFromPdfCommand, ErrorOr<GeneratedQuizResponse>>
{
    private const int MaxTextLengthForAi = 300_000;

    public async Task<ErrorOr<GeneratedQuizResponse>> Handle(GenerateAssociativeBushFromPdfCommand request, CancellationToken cancellationToken)
    {
        ...
        return await aiQuizGenerator.GenerateAssociativeBushAsync(
            extractedText,
