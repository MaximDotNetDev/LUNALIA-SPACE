...
public sealed class GenerateMatchingFromPdfCommandHandler(
    IPdfTextExtractionService pdfExtractor,
    IAiQuizGenerator aiQuizGenerator)
    : IRequestHandler<GenerateMatchingFromPdfCommand, ErrorOr<GeneratedQuizResponse>>
{
    private const int MaxTextLengthForAi = 300_000;

    public async Task<ErrorOr<GeneratedQuizResponse>> Handle(GenerateMatchingFromPdfCommand request, CancellationToken cancellationToken)
    {
        ...
        return await aiQuizGenerator.GenerateMatchingAsync(
            extractedText,
