...
public sealed class GenerateProofreaderFromPdfCommandHandler(
    IPdfTextExtractionService pdfExtractor,
    IAiQuizGenerator aiQuizGenerator)
    : IRequestHandler<GenerateProofreaderFromPdfCommand, ErrorOr<GeneratedQuizResponse>>
{
    private const int MaxTextLengthForAi = 300_000;

    public async Task<ErrorOr<GeneratedQuizResponse>> Handle(GenerateProofreaderFromPdfCommand request, CancellationToken cancellationToken)
    {
        ...
        return await aiQuizGenerator.GenerateProofreaderAsync(
            extractedText,
